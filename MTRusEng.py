...
class MTRusEng(DataModule):
    def __init__(self, batch_size, num_steps = 9, num_train=512, num_val = 128):
        super(MTRusEng, self).__init__()
    ...
    def _download(self):
            #Utility.download("https://www.manythings.org/anki/rus-eng.zip", self.root)
            #Utility.extract(self.root + '/rus-eng.zip')
            
            with open(self.root + '/rus.txt', encoding='utf-8') as f:
                return f.read()

    def _preprocess(self, text):
        # Replace non-breaking space with space
        text = re.sub(r'\tCC-BY.*', '', text)
